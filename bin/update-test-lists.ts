...
const fetchCoinMarketCapList = async (apiKey: string): Promise<string[]> => {
    ...
    const latestListingsResponse = await fetch(
        `https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?${latestListingsParams.toString()}`,
        {
            headers: {
    ...
    const metadataResponse = await fetch(
        `https://pro-api.coinmarketcap.com/v2/cryptocurrency/info?${metadataV2Params.toString()}`,
        {
            headers: {
                "X-CMC_PRO_API_KEY": apiKey,
            },
...
const fetchSnapsRegistryList = async (): Promise<string[]> => {
    const snapsRegistryResponse = await fetch("https://acl.execution.metamask.io/latest/registry.json", {
        headers: {
            "User-Agent":
                "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) EthPhishingDetect/1.2.0 Safari/537.36",
        },
    });
